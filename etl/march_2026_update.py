#!/usr/bin/env python3
"""
March 2026 Incremental Update ETL
===================================
Processes march-2026.xlsx as a DELTA UPDATE against data already uploaded
from the original jutem_fund.xlsx "MARCH 2026" sheet.

What already exists in Directus (from original upload):
  - 83 loans with amortization entries (all status=pending, expected_amount=None)

What this script does:
  1. Updates 83 existing amortization entries with expected_amount/profit values
  2. Updates 29 amortization statuses from pending → paid
  3. Creates payment records for the 29 newly-paid installments
  4. Creates 17 new loans (+ borrowers if needed) + amortization entries
  5. Updates loan statuses (active → settled) for fully-paid loans
  6. Uploads 19 interest expense records
  7. Uploads 115+1 other expenses + tracking fees

Usage:
    python3 etl/march_2026_update.py                    # Dry run (default)
    python3 etl/march_2026_update.py --execute          # Actually upload
    python3 etl/march_2026_update.py --step loans       # Loans + amortization + payments
    python3 etl/march_2026_update.py --step expenses    # Other expenses + tracking fees
    python3 etl/march_2026_update.py --step interest    # Interest expenses only
"""
...
EXCEL_FILE = os.path.join(os.path.dirname(__file__), "..", "march-2026.xlsx")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output", "march_2026")
...
def fetch_march_amortization(token):
    """Fetch ALL March 2026 amortization entries (no company field on this table)."""
    print("Fetching existing March 2026 amortization entries...")
    entries = api_get(token, "/items/amortization", {
        "filter[due_date][_gte]": "2026-03-01",
        "filter[due_date][_lte]": "2026-03-31",
        "fields": "*",
        "limit": -1,
    })
    print(f"  {len(entries)} existing amortization entries for March 2026")
    return entries
...
def process_loans(loans, token, execute=False):
    ...
    existing_amort = fetch_march_amortization(token)
    ...
    for idx, loan_row in enumerate(loans):
        ...
        if matched_amort:
            ...
            if old_status != new_status and new_status in ("paid", "cleared"):
                pmt_counter += 1
                ref = f"PMT-MAR26-{pmt_counter:05d}"
...
def process_interest_expenses(interest_expenses, token, execute=False):
    ...
    existing = api_get(token, "/items/transactions", {
        ...
        "filter[transaction_date][_gte]": "2026-03-01",
        "filter[transaction_date][_lte]": "2026-03-31",
        "fields": "id,reference_number,notes,amount",
        "limit": -1,
    })
    existing_refs = {e.get("reference_number", "") for e in existing}
    print(f"  {len(existing)} existing March 2026 interest expense transactions")
...
def process_expenses(other_expenses, tracking_fees, token, execute=False):
    ...
    print(f"  {len(existing)} existing March 2026 expense transactions")
...
def main():
    parser = argparse.ArgumentParser(description="March 2026 Jutem Fund Delta Update")
    ...
    print(f"\n{'='*60}")
    print(f"March 2026 Data Summary (from march-2026.xlsx)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"  Delta from original upload:")
    print(f"    ~83 existing amortization entries to update (expected_amount/profit)")
    print(f"    ~29 status changes (pending → paid) + payment records")
    print(f"    ~17 new loans + amortization + borrowers")
    print(f"    {len(interest_expenses)} interest expenses")
    print(f"    {len(other_expenses)+len(tracking_fees)} other expenses + tracking fees")
    print(f"{'='*60}")
    ...
    if not args.execute:
        ...
        print(f"  python3 etl/march_2026_update.py --execute")
        print(f"  # Or step by step:")
        print(f"  python3 etl/march_2026_update.py --step loans --execute")
        print(f"  python3 etl/march_2026_update.py --step interest --execute")
        print(f"  python3 etl/march_2026_update.py --step expenses --execute")
